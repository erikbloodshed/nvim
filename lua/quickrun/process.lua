...
Parameters:
  command_parts: table - A list of strings where the first element is the command
                         and subsequent elements are its arguments. (e.g., {'g++', '-o', 'main', 'main.cpp'})
  options: table | nil - An optional table for configuration with fields:
...
local ERROR_MESSAGES = {
  invalid_command_parts = 'Invalid command: expected a non-empty table of strings',
  invalid_command = 'Invalid command: the first element of the table must be a non-empty string',
...
M.execute = function(command_parts, options)
  -- Validate and normalize configuration
  options = options or {}
  local timeout_duration = (options.timeout and tonumber(options.timeout)) or DEFAULT_CONFIG.TIMEOUT_MS
  local resources = setmetatable({ handles = {} }, { __index = ResourceManager })
  local result = { code = -1, stderr = '', error_type = nil }

  -- Input validation
  if type(command_parts) ~= 'table' or #command_parts == 0 then
    result.stderr = ERROR_MESSAGES.invalid_command_parts
    result.error_type = ERROR_TYPES.VALIDATION
    return result
  end

  -- The command is the first part, args are the rest.
  -- Create a copy to avoid modifying the caller's table.
  local cmd_copy = {}
  for i = 1, #command_parts do
    cmd_copy[i] = tostring(command_parts[i]) -- Ensure all parts are strings
  end

  local command_path = table.remove(cmd_copy, 1)
  local command_args = cmd_copy
