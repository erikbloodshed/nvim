...
function StatusLine.new()
  return setmetatable({
    ...
    components = {},
    win_data = setmetatable({}, { __mode = "k" }),
    width_cache = setmetatable({}, { __mode = "k" }),
    titles_tbl = {
...
function StatusLine:get_win_cache(winid)
  local d = self.win_data[winid]
  if not d then
    d = { cache = CacheManager.new(), git = {}, icons = {} }
    self.win_data[winid] = d
  end
  return d.cache
end

function StatusLine:is_excluded_buftype(win)
  if not api.nvim_win_is_valid(win) then return false end
  local bo = vim.bo[api.nvim_win_get_buf(win)]
  return self.config.exclude.buftypes[bo.buftype] or self.config.exclude.filetypes[bo.filetype]
end

function StatusLine:refresh_win(winid)
  if not api.nvim_win_is_valid(winid) then
    self.win_data[winid] = nil
    return
  end
  vim.wo[winid].statusline = string.format("%%!v:lua.require'ui.statusline'.status(%d)", winid)
end

function StatusLine:conditional_hl(content, hl, apply_hl)
  if not apply_hl or not hl or not content then return content or "" end
  return string.format("%%#%s#%s%%*", hl, content)
end

function StatusLine:register_component(name, render_fn, opts)
  opts = opts or {}
...
function StatusLine:create_context(winid, bufnr)
  return {
    winid = winid,
    bufnr = bufnr,
    cache = self:get_win_cache(winid),
    wdata = self.win_data[winid],
    bo = vim.bo[bufnr],
  }
end

function StatusLine:render_component(name, ctx, apply_hl)
...
function StatusLine:get_width(str)
  if not str or str == "" then return 0 end
  if self.width_cache[str] then return self.width_cache[str] end
  local cleaned = str:gsub("%%#[^#]*#", ""):gsub("%%[*=<]", "")
  local width = fn.strdisplaywidth(cleaned)
  self.width_cache[str] = width
  return width
end

function StatusLine:assemble(parts, sep)
  local tbl = {}
...
function StatusLine:render(winid)
  local bufnr = api.nvim_win_get_buf(winid)
  if self:is_excluded_buftype(winid) then
    local ctx = self:create_context(winid, bufnr)
    return "%=" .. self:render_component("simple_title", ctx, true) .. "%="
  end
  local apply_hl = winid == api.nvim_get_current_win()
  local ctx = self:create_context(winid, bufnr)
  local sep = self:conditional_hl(self.config.seps, "StatusLineSeparator", apply_hl)
  local left = self:assemble({
    self:render_component("mode", ctx, apply_hl),
    self:render_component("directory", ctx, apply_hl),
    self:render_component("git_branch", ctx, apply_hl),
  }, sep)
  local right = self:assemble({
    self:render_component("diagnostics", ctx, apply_hl),
    self:render_component("lsp_status", ctx, apply_hl),
    self:render_component("position", ctx, apply_hl),
    self:render_component("percentage", ctx, apply_hl),
  }, sep)
  local center = self:assemble({
    self:render_component("file_display", ctx, apply_hl),
    self:render_component("file_status", ctx, apply_hl),
  }, " ")
  local w_left, w_right, w_center, w_win = self:get_width(left), self:get_width(right), self:get_width(center),
...
function StatusLine:invalidate_and_refresh(buf, keys)
  for _, winid in ipairs(fn.win_findbuf(buf)) do
    if self.win_data[winid] then
      self:get_win_cache(winid):invalidate(keys)
    end
    self:refresh_win(winid)
  end
end

function StatusLine:setup_autocmds()
  ...
  autocmd({ "BufWinEnter", "BufWritePost" }, {
    group = group,
    callback = function(ev) self:invalidate_and_refresh(ev.buf, all_keys) end,
  })
  autocmd("BufModifiedSet", {
    group = group,
    callback = function(ev) self:invalidate_and_refresh(ev.buf, "file_status") end,
  })
  autocmd("DirChanged", {
    group = group,
    callback = function(ev) self:invalidate_and_refresh(ev.buf, { "directory", "git_branch" }) end,
  })
  autocmd("DiagnosticChanged", {
    group = group,
    callback = function(ev) self:invalidate_and_refresh(ev.buf, "diagnostics") end,
  })
  autocmd({ "LspAttach", "LspDetach" }, {
    group = group,
    callback = function(ev) self:invalidate_and_refresh(ev.buf, "lsp_clients") end,
  })
  ...
  autocmd({ "WinEnter", "WinLeave" }, {
    group = group,
    callback = function()
      for _, w in ipairs(api.nvim_list_wins()) do self:refresh_win(w) end
    end,
  })
  autocmd("WinClosed", {
    group = group,
    callback = function(ev)
      ...
      if winid then self.win_data[winid] = nil end
    end,
  })
end

local function register_components(statusline)
  statusline:register_component("mode", function(_, apply_hl)
    local mode_info = statusline.modes_tbl[api.nvim_get_mode().mode]
    return statusline:conditional_hl(mode_info.text, mode_info.hl, apply_hl)
  end)

  statusline:register_component("directory", function(ctx, apply_hl)
    ...
    return statusline:conditional_hl(content, "Directory", apply_hl)
  end, { cache_keys = { "directory" } })

  statusline:register_component("git_branch", function(ctx, apply_hl)
      ...
      if ctx.wdata.git[cwd] == nil then
        ...
        vim.system({ "git", "branch", "--show-current" }, { cwd = cwd, text = true, timeout = 1000 },
          vim.schedule_wrap(function(result)
            ...
            statusline:refresh_win(ctx.winid)
          end))
      end
      return ctx.wdata.git[cwd] or ""
    end)
    if branch_name and branch_name ~= "" then
      return statusline:conditional_hl(icons.git .. " " .. branch_name, "StatusLineGit", apply_hl)
    end
    return ""
  end, { cache_keys = { "git_branch" } })

  statusline:register_component("file_display", function(ctx, apply_hl)
    ...
    if file_data.icon and file_data.icon ~= "" then
      parts[#parts + 1] = statusline:conditional_hl(file_data.icon, file_data.hl, apply_hl)
      parts[#parts + 1] = " "
    end
    parts[#parts + 1] = statusline:conditional_hl(file_data.name, "StatusLine", apply_hl)
    return table.concat(parts, "")
  end, { cache_keys = { "file_data" } })

  statusline:register_component("file_status", function(ctx, apply_hl)
    ...
    if status.readonly then
      return statusline:conditional_hl(icons.readonly, "StatusLineReadonly", apply_hl)
    elseif status.modified then
      return statusline:conditional_hl(icons.modified, "StatusLineModified", apply_hl)
    end
    return " "
  end, { cache_keys = { "file_status" } })

  statusline:register_component("simple_title", function(ctx, apply_hl)
    local title = statusline.titles_tbl.buftype[ctx.bo.buftype] or statusline.titles_tbl.filetype[ctx.bo.filetype]
    return statusline:conditional_hl(title or "no file", "String", apply_hl)
  end)

  statusline:register_component("diagnostics", function(ctx, apply_hl)
    ...
    if not counts or vim.tbl_isempty(counts) then
      return statusline:conditional_hl(icons.ok, "DiagnosticOk", apply_hl)
    end
    local parts = {}
    for _, diag in ipairs(statusline.diagnostics_tbl) do
      local count = counts[diag.severity_idx]
      if count and count > 0 then
        parts[#parts + 1] = statusline:conditional_hl(string.format("%s:%d", diag.icon, count), diag.hl, apply_hl)
      end
    end
    return table.concat(parts, " ")
  end, { cache_keys = { "diagnostics" } })

  statusline:register_component("lsp_status", function(ctx, apply_hl)
    ...
    return statusline:conditional_hl(content, "StatusLineLsp", apply_hl)
  end, { cache_keys = { "lsp_clients" } })

  statusline:register_component("position", function(_, apply_hl)
    return statusline:conditional_hl("%l:%v", "StatusLineValue", apply_hl)
  end)

  statusline:register_component("percentage", function(_, apply_hl)
    local mode_info = statusline.modes_tbl[api.nvim_get_mode().mode]
    return statusline:conditional_hl(" %P ", mode_info.hl, apply_hl)
  end)
end

local statusline = StatusLine.new()
register_components(statusline)
statusline:setup_autocmds()

return {
  status = function(winid) return statusline:render(winid) end,
}
