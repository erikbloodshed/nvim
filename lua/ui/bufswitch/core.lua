...
local function end_cycle()
  ...
  if config.show_tabline then
    ui.update(M.state.tabline_order, nil, should_apply_hl())
  end
end
...
function M.navigate(move)
  ...
  local apply_hl = should_apply_hl()

  if not M.state.cycle.active then
    if #M.state.tabline_order < 2 then
      ui.show_temp(M.state.tabline_order, apply_hl)
      return
    end
  ...
  if move == "recent" then
    ...
    if M.state.cycle.index < 1 then
      if not config.wrap_around then
        ui.show_temp(M.state.tabline_order, apply_hl); return
      end
  ...
  vim.cmd('buffer ' .. t)
  vim.o.showtabline = 2
  ui.update(M.state.tabline_order, M.state.cycle.index, apply_hl)
