...
local highlights = {
    ...
    ["@comment.error"]                         = { fg = colors.bg0, bg = colors.dark_red, bold = true },
    ["@comment.note"]                          = { fg = colors.bg0, bg = colors.dark_cyan, bold = true },
    ["@comment.todo"]                          = { fg = colors.bg0, bg = colors.dark_yellow, bold = true },
...
vim.api.nvim_create_autocmd("LspTokenUpdate", {
    callback = function(args)
        ...
        -- Get all treesitter captures at the token's position
        local captures = vim.treesitter.get_captures_at_pos(args.buf, token.line, token.start_col)
        local target_capture = "variable.builtin"

        for _, capture in ipairs(captures) do
            -- Check if the token is the specific capture we want to prioritize
            if capture.capture == target_capture then
                vim.lsp.semantic_tokens.highlight_token(
                    token,
                    args.buf,
                    args.data.client_id,
                    "@" .. target_capture,
                    { priority = 126 } -- Set the desired priority directly
                )
                -- We found and highlighted our target, no need to check other captures
                break
            end
        end
    end,
...
-- -- Setting more priority to treesitter highlight
-- local boost = { { treesitter = "variable.builtin", priority = 126 }, }
--
